...
*     16-APR-2026 (DSB):
*        Raise the near-zero floor in astEQUALS from DBL_MIN to 1.0e-12
*        so the helper reports equality for pairs of values of order
*        DBL_EPSILON (for example, the ulp-scale inverse-matrix residuals
*        that drift between macOS and Linux math libraries).
*     4-JUN-2026 (EMB):
*        Change the AST__DBL_DIG fallback (used when DBL_DECIMAL_DIG is
*        not exposed, e.g. a C99 build) from DBL_DIG + 3 to DBL_DIG + 2,
*        so it equals DBL_DECIMAL_DIG for IEEE-754 binary64. Serialised
...
/* Number of digits to use when formatting double precision floating point
   ...
   text to binary, so we use DBL_DECIMAL_DIG instead. DBL_DECIMAL_DIG is
   only exposed by <float.h> under C11 and later, so when it is not
   available (e.g. a C99 build) we fall back to DBL_DIG + 2, which equals
   DBL_DECIMAL_DIG for IEEE-754 binary64. Keeping the two paths equal
   means the serialised number of digits (and hence the exact text of
   dumped Objects) is identical regardless of the C standard AST was
   compiled against. */
#ifdef DBL_DECIMAL_DIG
  #define AST__DBL_DIG (DBL_DECIMAL_DIG)
#else
  #define AST__DBL_DIG (DBL_DIG + 2)
#endif
...
/* Check for equality of floating point values. We cannot compare bad values
   ...
   values are dealt with explicitly. The comparison combines a relative
   tolerance (suitable for values that are not close to zero) with an
   absolute floor of 1.0e-12 so that near-zero values, where the relative
   formula degenerates to DBL_EPSILON * near-zero ~ 1e-32, still compare
   equal under realistic arithmetic noise (for example, off-diagonal
